...
def loadresults(c):
    ...
    """
    #Path names
    samples = pd.read_csv('sample_names.txt', 
                          header = None,
                          comment ='#',   # Skip lines starting with '#'
                          skip_blank_lines = True
                          )
    
    Names   = pd.read_csv('formal_names.txt', 
                          header=None,
                          comment ='#',   # Skip lines starting with '#'
                          skip_blank_lines = True
                          )
    ...
    for i in range(len(samples)):
        data[samples[0][i]] = json.load(open(str(RESULTS_DIR) + '/' + samples[0][i] + ".json"))
...
def loadresults_(c, samples, name):
    """
    Load .json load_results
    """
    #Path names
...
def loadresults2(c):
    ...
    samples = pd.read_csv('sample_names.txt', header=None)
    Names   = pd.read_csv('formal_names.txt', header=None)
