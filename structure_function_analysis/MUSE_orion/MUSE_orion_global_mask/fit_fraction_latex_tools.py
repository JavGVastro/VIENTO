...
def _fit_triplet(results: Mapping, parameter: str) -> tuple[float, float, float]:
    """Return central, upper, and lower values from an entry or raw bundle."""
    fit = results.get("fit")
    if isinstance(fit, pd.DataFrame) and parameter in fit:
        values = fit[parameter].tolist()
    else:
        values = results.get("fit_dict", {}).get(parameter)
    if values is None or len(values) < 3:
        raise KeyError(
            f"Result has no complete [value, upper, lower] triplet for "
            f"{parameter!r}."
        )
    triplet = tuple(float(value) for value in values[:3])
    if not np.all(np.isfinite(triplet)):
        raise ValueError(f"Non-finite values for {parameter!r}: {triplet!r}")
    return triplet
...
def get_fit_result(
    ...
    *,
    ...
    """Return exactly one selected/averaged entry for a group, bin, and line."""
    matches = [
        entry
        for entry in data_in
        if entry.get("group") == group
        and entry.get("bin") == bin_level
    ...
    available = sorted(
        {
            (entry.get("group"), entry.get("bin"), entry.get("line"))
            for entry in data_in
        },
    ...
    if not matches:
        raise KeyError(
            f"No result for group={group!r}, bin={bin_level!r}, "
            f"line={line!r}. Available combinations: {available}"
        )
    raise ValueError(
        f"Found {len(matches)} results for group={group!r}, "
        f"bin={bin_level!r}, line={line!r}; expected exactly one."
    )


def _validate_pc_summary_metadata(results: Mapping) -> tuple[float, float, float]:
    meta = results.get("meta", {})
    if meta.get("s0_unit") == "arcsec":
        raise ValueError(
            "The LaTeX summary requires s0 in pc. Pass selected_results_pc "
            "or another result before convert_s0_pc_to_arcsec()."
        )
    missing = [key for key in ("BOX_SIZE", "PC", "PIX") if key not in meta]
    if missing:
        raise KeyError(f"Missing required metadata for LaTeX summary: {missing}")
    box_size = float(meta["BOX_SIZE"])
    pc = float(meta["PC"])
    pix = float(meta["PIX"])
    if not np.all(np.isfinite([box_size, pc, pix])) or pc <= 0:
        raise ValueError(
            f"Invalid BOX_SIZE/PC/PIX metadata: {(box_size, pc, pix)!r}"
        )
    return box_size, pc, pix


def format_fit_summary_row(
    results: Mapping,
    *,
    fwhm: float = 2.35,
    include_identifiers: bool = False,
    line_labels: Mapping[str, str] | None = None,
    row_terminator: bool = False,
) -> str:
    """Format one selected or averaged fit as the original LaTeX table row.

    ``results`` must still contain ``s0`` in pc.  When
    ``include_identifiers=True``, group, bin, line, and selected fit limits are
    prepended to the numerical columns.
    """
    box_size, pc, pix = _validate_pc_summary_metadata(results)
    sig2, sig2_plus, sig2_minus = _fit_triplet(results, "sig2")
    m, m_plus, m_minus = _fit_triplet(results, "m")
    r0, r0_plus, r0_minus = _fit_triplet(results, "r0")
    s0, s0_plus, s0_minus = _fit_triplet(results, "s0")
    noise, noise_plus, noise_minus = _fit_triplet(results, "noise")
    if np.isclose(r0, 0.0):
        raise ZeroDivisionError("Cannot format derived columns because r0 is zero.")
    ...
    columns = [
        ...
        #f"<{((s0 + s0_plus)*(fwhm/pc)):.2f}",
        f"{s0*(fwhm/pc):.2f}\\PM{{{s0_plus*(fwhm/pc):.2f}}}{{{s0_minus*(fwhm/pc):.2f}}}",
        #f"{box_size:.2f}",
       ...
       # (
       #     f"{s0 / pc * fwhm:.4f}"
       #     f"\\PM{{{s0_plus / pc * fwhm:.4f}}}"
       #     f"{{{s0_minus / pc * fwhm:.4f}}}"
       # ),
    ]

    if include_identifiers:
        line = results.get("line")
        line_label = (line_labels or {}).get(line, line)
        limits = results.get("fit_limits_used")
        if limits is None and results.get("fit_limit") is not None:
            limits = (float(results["fit_limit"]),)
        limit_label = ",".join(f"{float(value):.1f}" for value in (limits or ()))
        columns = [
            str(results.get("group", "")),
            str(results.get("bin", "")),
            str(line_label or ""),
            limit_label,
            *columns,
    ...
    row = " & ".join(columns)
    return row + (r" \\" if row_terminator else "")


def fit_summary_diagnostics(
    results: Mapping,
    *,
    fwhm: float = 2.35,
) -> dict[str, float]:
    """Return the four diagnostic quantities printed by the old notebook."""
    _, pc, pix = _validate_pc_summary_metadata(results)
    _, noise_plus, _ = _fit_triplet(results, "noise")
    noise, _, _ = _fit_triplet(results, "noise")
    s0, s0_plus, _ = _fit_triplet(results, "s0")
    r0, _, _ = _fit_triplet(results, "r0")
    if np.isclose(r0, 0.0):
        raise ZeroDivisionError("Cannot calculate Binsize/r0 because r0 is zero.")
    return {
        "noise_upper": noise + noise_plus,
        "s0_rms_upper_pc": s0 + s0_plus,
        "s0_fwhm_upper_arcsec": (s0 + s0_plus) / pc * fwhm,
        "binsize_over_r0": (pc * pix) / r0,
    }


def print_fit_summary_row(
    results: Mapping,
    *,
    fwhm: float = 2.35,
    include_identifiers: bool = False,
    ...
    """Print and return a LaTeX row plus the original diagnostic lines."""
    row = format_fit_summary_row(
        results,
        fwhm=fwhm,
    ...
    diagnostics = fit_summary_diagnostics(results, fwhm=fwhm)
    print(row)
    #print(f"noise      = {diagnostics['noise_upper']:.2f} (km/s)^2")
    #print(f"s0(rms)    = {diagnostics['s0_rms_upper_pc']:.4f} pc")
    #print(f"s0(FWHM)   = {diagnostics['s0_fwhm_upper_arcsec']:.2f} arcsec")
    #print(f"Binsize/r0 = {diagnostics['binsize_over_r0']:.3f}")
    return row


def format_fit_summary_rows(
    data_in: Sequence[Mapping],
    *,
    group: str,
    bin_level: int,
    ...
    **format_kwargs,
    ...
    """Format all available lines for one group/bin in a chosen order."""
    entries = [
        entry
        for entry in data_in
        if entry.get("group") == group and entry.get("bin") == bin_level
    ]
    if line_order is not None:
        order = {line: index for index, line in enumerate(line_order)}
        entries = sorted(entries, key=lambda entry: order.get(entry.get("line"), len(order)))
    if not entries:
        raise KeyError(f"No results for group={group!r}, bin={bin_level!r}.")
    return [format_fit_summary_row(entry, **format_kwargs) for entry in entries]
...
__all__ = [
    ...
    "fit_limit_tag",
    "fit_summary_diagnostics",
    "format_fit_summary_row",
    "format_fit_summary_rows",
    "get_fit_result",
